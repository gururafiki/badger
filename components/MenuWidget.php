...
class MenuWidget extends  Widget{
    ...
    public function run(){
        ...
        $text=$this->getMenuHtml($this->tree,$arr[2],$arr[3],$arr[4],'pro',$arr[6],$arr[7],$arr[8],$arr[9],$arr[5]);
        ...
        $this->tree = $this->getTree();
        $text=$this->getMenuHtml($this->tree,$arr[2],$arr[3],'gen',$arr[5],$arr[6],$arr[7],$arr[8],$arr[9],$arr[4]);
        if(strlen($text)>50)
        ...
        $this->tree = $this->getTree();
        $text=$this->getMenuHtml($this->tree,$arr[2],$arr[3],$arr[4],$arr[5],$arr[6],'type',$arr[8],$arr[9],$arr[7]);
        if(strlen($text)>50)
        ...
        if($arr[7]!=0){
            ...
            $this->tree = $this->getTree();
            $text=$this->getMenuHtml($this->tree,$arr[2],$arr[3],$arr[4],$arr[5],'size',$arr[7],$arr[8],$arr[9],$arr[6]);
            if(strlen($text)>50)
            ...
            $this->tree = $this->getTree();
            $text=$this->getMenuHtml($this->tree,'col',$arr[3],$arr[4],$arr[5],$arr[6],$arr[7],$arr[8],$arr[9],$arr[2]);
            if(strlen($text)>50)
            ...
            $this->tree = $this->getTree();
            $text=$this->getMenuHtml($this->tree,$arr[2],'spo',$arr[4],$arr[5],$arr[6],$arr[7],$arr[8],$arr[9],$arr[3]);
            if(strlen($text)>50)
    ...
    public function getMenuHtml($tree,$col,$spo,$gen,$brand,$size,$type,$byprice,$product_type,$current){
        $str='';
        foreach ($tree as $category){
            if(!isset($category['childs'])){
                ...
                if($where!=''){
                    ...
                    if($count>0){
                        $str.=$this->catToTemplate($category,$col,$spo,$gen,$brand,$size,$type,$byprice,$product_type,$count,$current);
                    }
                }
            }
            else{
                $current_cat=Category::findOne(['id'=>$current]);
                $buf = $this->catToTemplate($category,$col,$spo,$gen,$brand,$size,$type,$byprice,$product_type,$count,$current_cat->parent_id);
                if(strlen($buf)>500)
                    $str.=$buf;
            }
        }
    ...
    protected function catToTemplate($category,$col,$spo,$gen,$brand,$size,$type,$byprice,$product_type,$count,$current){
        ob_start();
